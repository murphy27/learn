...
import com.yun.learn.item.thread.thread_CyclicBarrier.TaskRunnable;
...
public class CompletableFutureDemo {
    public static void main(String[] args) {
        ...
        final List<Integer> taskList = Lists.newArrayList(2, 1, 3, 4, 5, 6, 7, 8, 9, 10, 2, 1, 3, 4, 5, 6, 7, 8, 9, 10, 2, 1, 3, 4, 5, 6, 7, 8, 9, 10, 2, 1, 3, 4, 5, 6, 7, 8, 9, 10);
        try {
            //方式一：循环创建CompletableFuture list,
            // 调用sequence()组装返回一个有返回值的CompletableFuture，返回结果get()获取
            for (int i = 0; i < taskList.size(); i++) {
                ...
                CompletableFuture<Integer> future = (CompletableFuture<Integer>) CompletableFuture.supplyAsync(new TaskFuture(taskList.get(i)), exs).whenComplete(
                        (v, e)->{
                            intList.add(v);
                        }
                );
            ...
            System.out.println("计算结果数"+intList.size());
            System.out.println("计算结果为和"+totle);
     ...
     * @param futures List
     * @return
     * @Description 组合多个CompletableFuture为一个CompletableFuture,
     * 所有子任务全部完成，组合后的任务才会完成。带返回值，可直接get.
     * @author diandian.zhang
